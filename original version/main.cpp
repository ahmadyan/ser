...
/*
	comments:
	first cycle		+ sys: timing_derating			(supports timing, logic & electrical masking )
					+ sim(MC): timing_derating_sim

	multi cycle		+ sys:		system_failure_clockC_multiprop
					+ sim(MC):	system_faulure_simulation
*/
...
#define MAX 1000 // Maximum nodes
#define MAX_GATE_INPUTS 20  // Maximum number of inputs of a typical gate
#define MAX_DFF 250
#define MAX_CLK 50
#define MAX_PO 50
#define MAX_CLK_SYS1 1
#define MAX_CLK_SYS2 3
#define MAX_CLK_SYS3 4
#define sp_error_tolerance  0.05	// old value=0.05f;
#define derating_sim_error_tolerance 0.01
...
struct gate
{
	...
	short* input_values;
	short out;
	short type; // indicates gate's type including NAND(0),AND(1),NOR(2),OR(3),XOR(4),XNOR(5),NOT(6),DFF(7)
	short value;
	long sum_values;
...
//float circuit_period;
int tperiod, tsu=18, th=18;
int seu_pulse_width = 50;
//int seu_pulse_width = 70; //0.125pC
//int seu_pulse_width = 170;	//0.15pC
//int seu_pulse_width = 280;	//0.175pC
bool variance_alarm = false;

// Function prototypes
short  readNetlist  (FILE *fp, short nodes); // reads circuit netilist and returns number of circuit nodes (nodes-1)
...
void logic_simulation(short run_for_sp); //Perfroms logic simulation on the gate_list
void compute_gate_value(struct gate* gateptr); //Computes the gate value
void compute_signal_probability(int step_iteration, long max_iteration); // Computes signal probabilities of the circuit
...
void system_failure_multi_cycle(short clkC);
void system_failure_simulation_with_let(long, long, long);
...
int main(int argc, char *argv[]){
	...
	cout << endl << "Maximum number of nets supported: " << MAX;
	cout << endl << "Please change MAX for larger circuits";
	cout << endl;
	argc = 2;
	//argv[1] = "EGF_NGF_PDGF_to_NFAT_PP2BactivatesNFAT.v";
	//argv[1] = "EGF_NGF_PDGF_to_NFAT_MoreMol.v";
	//argv[1] = "EGF_NGF_PDGF_to_NFAT.v";
	//argv[1] = "INSULIN_TNFalpha_to_CFOS.v";
	//argv[1] = "s386.v";
	//argv[1] = "s33.v";
	//argv[1] = "s298.v";
	//argv[1] = "s27.v";
	...
	//argv[1] = "stest2.v";
	if (argc <2){
		cout << "There is no file input!!! ==> Exit" << endl;
		exit(1);
	}
	char outfilename [200];
	strcpy(outfilename, "results_");
	strcat(outfilename, argv[1]);
	outfilename[strlen(outfilename)-2] = '\0';

	strcat(outfilename, "_mcycle_let");
	strcat(outfilename, ".txt");
	fp = fopen(argv[1], "r");    resultfp = fopen(outfilename, "w");

	// Exit if error in opening file
	if(fp == NULL) {
		cout << "Netlist not found" << endl;
		exit(1);
	}
	...
	nodes = readNetlist(fp, nodes);
	alloc_init_mem();	// Allocate initial memory

	gate_numbers_noff = gate_numbers - dff_num;
    arange_gates();
    //arange_gates_extract_feedbacks();
	cout << "Number of inputs:" << numPi << endl;
	fprintf(resultfp,"\nNumber of inputs: %d \n", numPi);
	cout << "Number of outputs:" << numPo << endl;
	fprintf(resultfp,"Number of outputs: %d \n", numPo);
	cout << "Number of flip-flops:" << dff_num << endl;
	fprintf(resultfp,"Number of flip-flops: %d \n", dff_num);
	cout << "Number of gates (including FFs):" << gate_numbers << endl;
    ...
    cout << endl << "Time required to extract netlist and arrange it is: " << total_time << " seconds"<<endl;
    fprintf(resultfp, "\nTime required to extract netlist and arrange it is: %f seconds.\n", total_time);

	long max_iteration = 2<<(numPi+dff_num-1);
    long sp_iteration=100000;
    long vector_iteration=100000;
	long max_derating_iteration=100;

    initial_time = clock();
    time(&t1);
    srandom(0);
	int step_iteration = 100; // 100 is minimum iteration for 0.05 error tolerance
    compute_signal_probability(step_iteration, sp_iteration);
    ...
    t3 = t2 - t1;
    if (t3>1000) total_time = t3;
    cout << endl << "Time required to compute signal probability is: " << total_time << " seconds"<<endl;
    //cout << "Number of iteration: " << sp_iteration << endl;
    fprintf(resultfp, "\nTime required to compute signal probability is: %f seconds.\n", total_time);
   

	sim_gate_numbers = gate_numbers;
    int max_gates_simulated = 20;
	if ((gate_numbers-dff_num) > max_gates_simulated)
		sim_gate_numbers = max_gates_simulated + dff_num;
	compute_circuit_delay(false);
	tperiod = (int)max_delay + tsu + th;
	fprintf(resultfp, "\n Circuit period = %d \n", tperiod);
	...
	//system_failure_simulation_with_let(vector_iteration, step_iteration, MAX_CLK);

	//system_failure_simulation_clockC(vector_iteration, MAX_CLK);
    final_time = clock();
    total_time = (double) (final_time - initial_time) / (double) CLOCKS_PER_SEC ;
    time(&t2);
    t3 = t2 - t1;
    if (t3>1000) total_time = t3;
    cout << endl << "SIMULATION Multicycle: Time required to compute Mont Carlo Simulation with LET. VECTOR PER GATE: " << total_time/sim_gate_numbers << " seconds"<<endl<<endl;
    fprintf(resultfp, "\nSIMULATION Multicycle: Time required to compute  Mont Carlo Simulation with LET. VECTOR PER GATE: %f seconds. total time=%f seconds\n\n", total_time/sim_gate_numbers, total_time);
	
	for(int i=10;i<15;i++){
		initial_time = clock();
		time(&t1);

		system_failure_multi_cycle(i);
		...
		t3 = t2 - t1;
		if (t3>1000) total_time = t3;
		fprintf(resultfp, "\nSys.Multicycle: TimePerGate=%f Time =%f seconds\n\n", total_time/sim_gate_numbers, total_time);
	}
	...
	//fprintf(resultfp, "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$\n");
	//system_failure_multi_cycle(10);
	//print_multi_cycle_results(10);
	...
	if (variance_alarm == true) fprintf(resultfp, "Variance ALARM \n");
	print_slack_info();
	fclose(resultfp);
	release_all_malloc();
	//system("PAUSE");
...
short readNetlist(FILE *fp, short nodes)
{
	...
	do
	{
	  // skip if x=3 (blank space, end of line, etc)
		do
		{
	  ...
	  // if 'module' - skip
		...
		// end of file
		else if(strcmp(field, "endmodule") == 0)
			break;
			//return nodes;
		// If 'input'
		else if(strcmp(field, "input") == 0)
		{
			cout << "starts inputs!" << endl;
			do
			{
				x = extractField(fp, field);
				if ((x==EOF)||(x<0))
		...
		else if(strcmp(field, "output") == 0)
		{
			...
			do {
				...
				if (x==3)
					continue;
				cout << endl << "Output: " << field;
				cout << " Mapping: " << nodes;
				fprintf(resultfp, "\n Output: %s  Mapping: %d", field, nodes);
		...
		else if(strcmp(field, "wire") == 0) 
		{
			...
			do
			{
				...
				if (x==3)
					continue;
				cout << endl << "Wire: " << field;
				cout << " Mapping: " << nodes;
				fprintf(resultfp, "\n Wire: %s  Mapping: %d", field, nodes);
		...
		else
		{
			...
			if ((x==EOF)||(x<0))
				break;;
			cout << endl << "Instance: " << field <<"::" << field_Gate_Instance_Name;
	...
	if (nodes>MAX)
	{
		cout << "MAX is not large enough!!! ==> Exit" << endl;
		exit(1);
	}
...
// Returns none
void logic_simulation(short run_for_sp)
{
...
////// Print result of logic simulation
    ...
    //        if (fadjlist_ptr[i]!=NULL)
		...
		for (i=0; i<gate_numbers; i++)
			fprintf(resultfp, "Node %4d = %d \n", gate_list[i]->out, gate_list[i]->value);
	...
	if (is_in_range==false)
		fprintf(resultfp, "ALARM: logic value is not 0 or 1! \n");
...
void compute_gate_value(struct gate* gateptr)
{
    ...
    switch (gateptr->type) {
        case 0:     //nand
                ...
                for (i=1; i<(gateptr->input_count); i++)
...
// Returns none
void compute_signal_probability(int step_iteration, long max_iteration){
    long i;
	...
	float error_tolerance = sp_error_tolerance;//0.05;
	long total_iteration = 0;

    // Initialize the sum_values
    if (PI_list_sp == NULL)
        PI_list_sp = new float[numPi+1];
...
//			step_iteration = 10000;
		cout << "simulated signal prob. for " << circuit_total_iteration << endl;
...
//    for (i= (numPi+1); i<nodes; i++)
	/*for (i=0; i<5; i++)
		gate_list[i]->signal_probability = 0.1 * i;
	*/
...
// Purpose:  initializ the gate delays
// Returns none
void initialize_gate_delay(struct gate *gptr1)
{
...
void compute_circuit_delay(bool slack_resizing)
{
	...
	for (i=0;i<gate_numbers;i++)
	{
		...
		if (gptr1->type==7) //dff
			continue;
		for(j=0; j<fadjlist[gptr1->out].size(); j++) //compute output load capacitance
		{
...
void old_timing_derating_code(short clkC){
	...
	for (ff_index=0; ff_index<gate_numbers; ff_index++)
	{
		...
		{
			gateptr = fadjlist_ptr[PI2POlist[i][0]];
			gateptr->p0 = MAX; gateptr->p1 = MAX;
			gateptr->pd = MAX; gateptr->pdn = MAX;
			gateptr->is_onpath = true;
		}
		gateptr = gate_list[ff_index];
		//gateptr->pd = 1; gateptr->pdn = 0; gateptr->p1 = 0; gateptr->p0 = 0;
		...
		add_event(gateptr,eventptr);
		eventptr = new struct event;
		eventptr->time = (int)gateptr->gd + seu_pulse_width;
		//APPROACH 1
	...
	for (ff_index=0; ff_index<gate_numbers; ff_index++){
		...
		for (int ck=1; ck<=clkC; ck++){
			if(ck==1){
				...
				for (i=0; i<PI2POlist_size; i++){
					gateptr = fadjlist_ptr[PI2POlist[i][0]];
					gateptr->q0 = MAX; gateptr->q1 = MAX;
					gateptr->qd = MAX; gateptr->qdn = MAX;
					gateptr->is_onpath = true;
				}
...
*/
				...
				for (int ff_ind=0; ff_ind<dff_num; ff_ind++){					
					...
					for(int i=0; i<gateptr->event_list.size()-1;i++){
						...
						//cout << ">>>>>..latch_prop===" << latch_prop << endl ;
				...
				//cout << ff_index << "  sys_failure=" << 1-sys_failure << endl ;
...
void compute_D_Probability_with_variance(struct gate* gateptr, float* p0, float* p1, float* pd, float* pdn, float* p0_var, float* p1_var, float* pd_var, float* pdn_var)
{
	...
	if ((gateptr->p0_var > 1) || (gateptr->p0_var < 0)) variance_alarm = true; //fprintf(resultfp, "Variance ALARM \n");
	if ((gateptr->p1_var > 1) || (gateptr->p1_var < 0)) variance_alarm = true; //fprintf(resultfp, "Variance ALARM \n");
	if ((gateptr->pd_var > 1) || (gateptr->pd_var < 0)) variance_alarm = true; //fprintf(resultfp, "Variance ALARM \n");
	if ((gateptr->pdn_var > 1) || (gateptr->pdn_var < 0)) variance_alarm = true; //fprintf(resultfp, "Variance ALARM \n");
...
// Returns none
void system_failure_simulation_with_let(long max_iteration, long step_iteration, long clk){
	cout << "-----------------------------------------------------------" << endl ;
	cout << "	system_failure_simulation_with_let" << endl ;
	cout << "-----------------------------------------------------------" << endl ;
	cout << "Number of FFs " << dff_num << endl;

	float EPP_PER_GATE = 0 ;
	float PO_PER_GATE_CLK1 = 0 ;
	
	int totalSEU_CLK1=0, totalFAILURE_CLK1=0, totalSim_CLK1=0;
	...
	for (ff_index=0; ff_index<sim_gate_numbers; ff_index++){
		...
		seu=0,lseu=0;
		cout << "Simulating gate# " << ff_index << " type= " << gate_name(gate_list[ff_index])<< " ... ";
					...
					if( ck==1 ){
						...
						for (j=0; j<gate_numbers; j++){
							...
							if (gate_list[j]->out==ff_index){
								int t1 = random() % tperiod; // circuit_period;
								//removing events >t1
								...
								//eventptr = (struct event*) malloc(sizeof(struct event));
								eventptr = new struct event;
								eventptr->time = t1+seu_pulse_width;
								//eventptr->p0 = 1; eventptr->p1=0; eventptr->pd=0; eventptr->pdn=0;
								eventptr->p0 = 1; eventptr->p1=0; eventptr->pd=0; eventptr->pdn=1;
								//eventptr->p0 =  1 - gateptr->signal_probability; eventptr->p1=gateptr->signal_probability; eventptr->pd=0; eventptr->pdn=0;
								add_event(gateptr,eventptr);
		...
		EPP_PER_GATE += ((1.0)*( totalSEU_CLK1 + totalFAILURE_CLK1 ))/ (1.0*totalSim_CLK1)  ;
		PO_PER_GATE_CLK1 += ((1.0)*( totalFAILURE_CLK1 ))/ (1.0*totalSim_CLK1)  ;
		S_sim_clk[ff_index] = sim_epp;
		S_sim_clk_var[ff_index] = sim_var;
		//printf( "simulated for: %d. first clock failure= %d\n", total_it, first_clock_failure);
		if(seu!=0) cout << " SEU=" << seu << " " ;
		if(lseu!=0) cout << " SET=" << lseu << " " ;
		cout << "simulated for: " << total_it << ". first clock failure= " << first_clock_failure << endl;
		fprintf(resultfp, "Node %d: derating= %f \n", ff_index , sim_epp);
	}
	...
	float sim_epp_total_clk1=0;
	float sim_epp_total_clk1_po=0;
	
	sim_epp_total = sim_epp_total/sim_gate_numbers;    
	sim_epp_total_clk1 = EPP_PER_GATE / sim_gate_numbers ;
	sim_epp_total_clk1_po = PO_PER_GATE_CLK1 / sim_gate_numbers ;
	   
	printf("average sim epp: %f \n", sim_epp_total);
	fprintf(resultfp, "average sim epp(clk1): %f \n", sim_epp_total_clk1);
	fprintf(resultfp, "average sim epp(clk1, POs Only): %f \n", sim_epp_total_clk1_po );
...
void print_multi_cycle_results(int clk)
{
	...
	int j=0;
	float diff_partial=0, diff_total=0, diff_total2=0;
	float sim_epp_total=0, sim_epp_no_po_total=0;
	float sys_epp_total=0, sys_epp_no_po_total=0;
	//short not_ff_po=0;
	short not_po=0;
			...
			//{
				...
				for (j=0; j<gate_numbers; j++)
				{
					...
					if (gate_list[j]->out==target_gate)
					{
						...
						t1 = random() % tperiod; // circuit_period;
						//removing events >t1
						list_size = gateptr->event_list.size();
...
void compute_gate_event_list(struct gate* gateptr)
{
	...
	int max_event_time = 3*tperiod + seu_pulse_width;
	int next_event;
	float vimin, vimax;
